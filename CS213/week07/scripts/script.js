...
// Store albums and their details
var albums = [
	[
...
function updateCart(albumNum){
	...
	if (items > 0)
		document.getElementById('coButton').style.display = "inline";
	else
...
// Validates dropdown for card selection
...
// When on order page, alert will show that form was reset
function resetForm(){
	alert("Form was reset: Items still in cart");
}

// Will show when order is submitted
function submitForm(){
	alert("Order submitted\nTotal: " + tax());
}

// This will reset on onLoad()
function resetForms(){
